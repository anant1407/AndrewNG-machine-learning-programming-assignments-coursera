...
H=X*Theta';
%cost=cost+lambda*(sum(X(i,:).^2))+ lambda*(sum(Theta(j,:).^2));

for i=1:num_movies
  for k=1:num_features
    regcostX= regcostX+X(i,k)^2;
  endfor
endfor

for j=1:num_users
  for k=1:num_features
    regcostTheta = regcostTheta+Theta(j,k)^2;
  endfor
endfor
...
J=cost/2+lambda*regcostX/2+lambda*regcostTheta/2;
